...
public class JyperionListener implements ITestListener {
	...
	public void onStart(ITestContext context) {
		log("onStart("+context+")");
		try {
			PdfWriter.getInstance(this.document, new FileOutputStream("Results/"+context.getName()+".pdf"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.document.open();
		...
		Paragraph p2 = new Paragraph("By: Md Syful Islam Jamaddar",
	...
	public static String takeScreenShot(WebDriver driver,
		...
		try {
			File file = new File("Results" + fileSeperator + "Screenshots");
			if (!file.exists()) {
			...
			File targetFile = new File("Results" + fileSeperator + "Screenshots" + fileSeperator + testName, screenShotName);
